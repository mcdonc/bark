...
| `LOGFIRE_TOKEN`             |                                      | Pydantic Logfire write token (opt-in; enables FastAPI auto-instrumentation when set)                                                                      |
...
- **Add favicon.ico**: The app has no favicon — browsers show a default blank icon. Add a Bark-themed favicon (paw icon) to `src/frontend/web/`.
...
- **Improve Dart code unit test coverage**: Add a coverage report to the Flutter test suite (similar to Python's `--cov-fail-under=100`) and increase coverage. Key untested files include `workspace_page.dart` (depends on `bark_plugins` codegen), `app.dart` (needs GoRouter/navigation mocking), and `main.dart`. Set up `flutter test --coverage` with `lcov` or similar tooling to generate and enforce a coverage threshold.
