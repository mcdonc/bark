...
- **Increase E2E parallelism**: Tests run with `fullyParallel: true` and 4 workers (down from 6.8 min to ~1.9 min). Port allocation race fixed with asyncio lock. Higher worker counts (8+) cause timing failures — concurrent Docker containers + Chrome instances + Ollama API calls saturate the system. Possible improvements: increase fixed wait times for container readiness, add retry/polling instead of `waitForTimeout`, or use `workers: "50%"` to scale with CI runner size.
