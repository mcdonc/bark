...
- **E2E: strengthen queued prompt test**: The current test only verifies that both prompts eventually get assistant responses. It should also verify: (1) the second prompt's user message is marked as queued (`is_queued` flag in the messages API), (2) the "queued" badge appears on the user message bubble in the UI, (3) the second assistant response appears only after the first one completes (ordering). Currently it's possible for both prompts to be processed non-queued if the first finishes before the second is sent.
