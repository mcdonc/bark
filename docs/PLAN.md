...
bark/
  ...
  src/dockerimage/
    Dockerfile                  # Pi agent image: node:22-slim + Pi + Python3 + build-essential + SQLite + vim + emacs + net tools
...
- **E2E: per-test user isolation**: Each E2E test should register and use its own unique user instead of sharing the `admin` user. Currently `stop_user_containers` on logout kills ALL containers for that user, breaking other parallel tests. Work started in a git stash — `createAndOpenWorkspace` registers a user internally, `login()` takes explicit credentials, each test passes a unique username. Still needs: container lifecycle test, two-workspaces test, and API-only tests to use their own users.
- **E2E: don't use registration form to create users in tests**: Tests should use the API to create users instead of the Flutter registration form, avoiding coordinate-based clicks and making tests more reliable.
- **Increase E2E parallelism**: Tests run with `fullyParallel: true` and 4 workers (down from 6.8 min to ~1.9 min). Port allocation race fixed with asyncio lock. At 8+ workers, the logout test's `stop_user_containers` kills other tests' containers (fixed by per-test user isolation above). Terminal-based tests also fail at high concurrency because the terminal isn't interactive yet when `terminalType` starts typing.
...
- **Docker base image**: Create a separate base image containing everything except Pi extensions, plugin tools, and the entrypoint (i.e., the apt packages, Node.js, Pi agent npm install). This layer changes rarely and could be pre-built and pushed to a registry, making workspace image builds near-instant (just COPY extensions + tools + entrypoint on top). Would also help CI caching since only the thin top layer needs rebuilding when plugins change.
