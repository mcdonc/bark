...
- **Rename OLLAMA*\* env vars to LLM*\***: The `OLLAMA_BASE_URL`, `OLLAMA_MODEL`, and `OLLAMA_API_KEY` env vars are misleading now that Bark supports any OpenAI-compatible LLM provider via the nginx proxy, not just Ollama. Rename to `LLM_BASE_URL`, `LLM_MODEL`, and `LLM_API_KEY`. Affected files: `.env`, `devenv.nix`, `scripts/nginx.sh`, `container_manager.py`, `env_util` references, docs in PLAN.md, and any deployment configs (arctor `.env`, etc.).
