...
- `bark exec WORKSPACE COMMAND...` — raw command execution in containers via WebSocket (`ExecSession` with piped stdin/stdout, no PTY)
- `bark sync SRC DEST` — rsync wrapper using `bark exec` as the transport
