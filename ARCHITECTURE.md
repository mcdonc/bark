...
3. **No per-container LLM config**: The backend injects `KLANGK_LLM_PROXY_URL=http://host.docker.internal:<nginx_port>/llm-proxy` into each container. `clank.sh` uses this to write Pi's `models.json` with the proxy URL (no real API key). `KLANGK_LLM_BASE_URL` is only used by nginx itself.
...
klangk/
  ...
  .env                          # Secrets: KLANGK_LLM_API_KEY, KLANGK_LLM_BASE_URL, KLANGK_LLM_MODEL, KLANGK_JWT_SECRET, etc.
  .gitignore
  README.md
  bootstrap                     # Install Nix + devenv

  plugins/              # Starter plugins (source of truth, fetched into $KLANGK_PLUGINS_DIR by update-plugins)
    celebrate/                  # Confetti animation (client-side)
    beep/                       # Audible beep tone (client-side)
    browser-fetch/              # Authenticated browser fetch (client-side)
    pig-latin/                  # Text to Pig Latin converter (server-side)
    word-count/                 # Fast file stats (server-side)
  scripts/
    import_dart_plugins.py     # Codegen: generates $KLANGK_PLUGINS_DIR/.dart/ package with plugin deps
    update_plugins.py          # Fetches plugins from git repos, writes plugins.lock
    stub_dart_plugins.sh       # Creates minimal klangk_plugins stub for first-time checkout / CI
  ...
  src/docker/
    Dockerfile                  # Workspace image: FROM klangk-base + plugin extensions + tools + npm deps + entrypoint + /etc/bash.bashrc
    Dockerfile.base             # Base image: node:26-slim + Pi + Python3 + build-essential + SQLite + vim + emacs + net tools + /bin/sh→bash (pushed to GHCR)
    entrypoint.sh               # Container init: chown, Claude Code skill symlinks, readiness flag
    clank.sh                    # Default workspace command: sets up Pi agent config, merges settings, builds system prompt, launches Pi with session resume and skill flags
    system-prompt.md            # Static system prompt for Pi (copied into image)
    bash.bashrc                 # System-wide bash defaults: Nix profile, readiness wait, default command exec
    builtin-extensions/         # Built-in Pi extensions (port-map.ts) — not from plugins
    # Plugin extensions and tools are staged at $KLANGK_PLUGINS_DIR/.docker/ at build time via named Docker build contexts

  src/backend/
    pyproject.toml              # Python deps: fastapi, aiodocker, aiosqlite, bcrypt, python-jose
    klangk_backend/
      cli/                      # CLI tool (klangk command) — typer app, thin HTTP+WebSocket client
        ...
        client.py               # KlangkClient (HTTP), WebSocket shell/exec session forwarding
        config.py               # Config storage (~/.config/klangk/cli.toml)
  ...
  src/frontend/
    ...
    lib/
      ...
      theme/
        colors.dart             # KColors: centralized dark theme color palette
      utils/
      ...
      widgets/
        klangk_logo.dart          # Klangk logo widget (green square, white robot icon)
        app_bar_title.dart        # Shared app bar title (logo + separator + back arrow + page name)
        app_bar_actions.dart      # Shared app bar actions (email chip + admin/settings/logout icons)
  ...
  src/frontend/e2e-tests/                # Playwright E2E tests with isolated test server
    e2e/klangk.spec.ts            # E2E test specs
    global-setup.ts             # Starts isolated Klangk server for E2E
