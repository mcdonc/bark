...
Pi containers access the LLM via the **LLM proxy**, an nginx location that proxies `/llm-proxy/` requests to `${LLM_BASE_URL}`. This is required because:
...
2. **API key security**: The API key is sent in a request header by the nginx proxy rather than being baked into the container image or passed as an env var. The `models.json` FIFO contains only the API key (written by the backend, read by Pi, then deleted).
3. **LLM_BASE_URL doesn't need to change**: The Pi configuration uses `${LLM_BASE_URL}` directly, so no per-container configuration is needed — nginx handles the routing.
