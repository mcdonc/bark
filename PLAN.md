...
- **Tool reminder on session resume**: When resuming an old Pi session, the LLM may not know about newly added extension tools. Attempted injecting a background prompt after `switch_session` but this caused race conditions with user prompts. Better approaches: inject tool awareness into Pi's system prompt via AGENTS.md or a Pi extension that modifies the system prompt on load, rather than as a conversation message.
