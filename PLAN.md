...
- **Terminal scrollbar and font rendering**: The terminal pane (xterm.dart) currently has no visible scrollbar for scrollback history. Font rendering on Flutter Web with CanvasKit has character spacing issues — characters appear with extra kerning. Investigate using a bundled monospace font, switching to the HTML renderer if Flutter supports it, or replacing xterm.dart with xterm.js via HtmlElementView for better web rendering.
